...
from db import db_connect, translate_table, translate_placeholder, fetchdf as db_fetchdf, DB_TYPE
...
def Tool_function_get_industry_members__(industry_name__):
    ...
    if industry_name__ not in Dict_cached_industry_member__:
        try:
            ...
            if DB_TYPE == 'doris':
                cursor = conn.cursor()
                sql = translate_table('SELECT ts_code FROM "基本信息表" WHERE industry = ?')
                sql = translate_placeholder(sql)
                cursor.execute(sql, [industry_name__])
...
def build_dsl_from_form_params(Parameter_dict__, metric_conditions):
    """把表单筛选参数翻译为一段 DSL 文本（由 dsl_engine 编译执行）。
    仅处理「股票」类型；空维度省略，AND 连接。返回 (dsl_text, concept_ignored, warnings)。
    未识别的枚举值不静默丢弃，记入 warnings 供调用方写日志。"""
    parts = []
    warnings = []
    stock = Parameter_dict__['股票']

    # 交易所板块：上证A股/深证A股/北证A股 -> 交易所 IN (...)；主板类 -> 板块 IN (...)
    exchange_list = stock['交易所板块']
    concept_ignored = False
    if exchange_list and '全部' not in exchange_list:
        ...
        for board in exchange_list:
            if board == '上证A股':
                ...
                ex_codes.append('SZSE')
            elif board == '北证A股':
                ex_codes.append('BSE')
            elif board in ('上证主板', '科创板', '深证主板', '创业板'):
                sectors.append(board)
            else:
                warnings.append(f'未识别的交易所板块选项 "{board}"，已忽略')
        terms = []
        if ex_codes:
            quoted = ','.join(f'"{c}"' for c in ex_codes)
            terms.append(f'交易所 IN ({quoted})')
        if sectors:
            quoted = ','.join(f'"{s}"' for s in sectors)
            terms.append(f'板块 IN ({quoted})')
        if terms:
            parts.append('(' + ' OR '.join(terms) + ')')

    # 概念：数据库无 concept 列，静默忽略
    concept_list = stock['概念']
    if concept_list and '全部' not in concept_list:
        concept_ignored = True

    # 行业
    industry_list = stock['行业']
    if industry_list and '全部' not in industry_list:
        quoted = ','.join(f'"{i}"' for i in industry_list)
        parts.append(f'行业 IN ({quoted})')

    # 地域
    area_list = stock['地域']
    if area_list and '全部' not in area_list:
        quoted = ','.join(f'"{a}"' for a in area_list)
        parts.append(f'地域 IN ({quoted})')

    # 风险警示：ST=含ST(含*ST)，*ST=纯*ST
    risk_list = stock['风险警示']
    if risk_list and '全部' not in risk_list:
        risk_set = set(risk_list)
        if risk_set == {'非风险警示股'}:
            parts.append('NOT ST')
        elif risk_set == {'ST股'}:
            parts.append('ST AND NOT *ST')
        elif risk_set == {'*ST股'}:
            parts.append('*ST')
        elif risk_set == {'非风险警示股', 'ST股'}:
            parts.append('NOT *ST')
        elif risk_set == {'非风险警示股', '*ST股'}:
            parts.append('NOT (ST AND NOT *ST)')
        elif risk_set == {'ST股', '*ST股'}:
            parts.append('ST')
        elif risk_set != {'非风险警示股', 'ST股', '*ST股'}:
            warnings.append(f'未识别的风险警示组合 {sorted(risk_set)}，已忽略')
        # 含「全部」或三选全选则不加条件

    # 名称关键字
    name_kws = stock['名称（关键字）']
    if name_kws and None not in name_kws:
        kws = [k for k in name_kws if k]
        if kws:
            or_parts = ' OR '.join(f'名称 包含 "{k}"' for k in kws)
            parts.append('(' + or_parts + ')' if len(kws) > 1 else or_parts)

    # 指标范围：DSL 总市值/流通市值内部已 /10000，输入用亿元值
    if metric_conditions:
        ...
        for metric_name, bounds in metric_conditions.items():
            if metric_name not in metric_dsl:
                warnings.append(f'未识别的指标 "{metric_name}"，已忽略')
                continue
            field = metric_dsl[metric_name]
            lo = bounds.get('下限')
            hi = bounds.get('上限')
            if lo is not None:
    ...
    return ' AND '.join(parts), concept_ignored, warnings


def execute_form_filter_as_dsl(Parameter_dict__, metric_conditions, log_messages__):
    """用 DSL 引擎执行表单筛选，返回 (rows, sec_type)。"""
    def log__(msg):
    ...
    dsl_text, concept_ignored, ignored_warnings = build_dsl_from_form_params(Parameter_dict__, metric_conditions)
    if concept_ignored:
        log__("概念筛选功能暂未支持（数据库无 concept 字段），将忽略此条件")
    for w in ignored_warnings:
        log__(f"【注意】{w}")
    if dsl_text.strip():
        log__(f"生成 DSL 指令：{dsl_text}")
        result = execute_dsl(dsl_text, dialect=DB_TYPE)
        log__(f"DSL 筛选完成（{result['dialect']} 方言），共选出 {result['count']} 只股票")
        return result['rows'], sec_type
    log__("无筛选条件，返回全部股票")
    result = execute_dsl('1 = 1', dialect=DB_TYPE)
    log__(f"DSL 筛选完成（{result['dialect']} 方言），共选出 {result['count']} 只股票")
    return result['rows'], sec_type
...
def Main_function_Get_security_histinfo_json__(Parameter_info_tuple__, Parameter_K_line_type__, Parameter_dir_path__):
    ...
    try:
        ...
        # ---------- 2. 从数据库读取日K数据 ----------
        if DB_TYPE == 'doris':
            sql_daily_k = translate_placeholder(translate_table('''
            ...
            '''))
            cursor = conn.cursor()
            cursor.execute(sql_daily_k, [ts_code__])
            df_daily_db = db_fetchdf(cursor)
        else:
        ...
        if Parameter_K_line_type__ == '101':  # 日K
            ...
            if DB_TYPE == 'doris':
                sql_weekly = translate_placeholder(translate_table('''
                ...
                '''))
                cursor = conn.cursor()
                cursor.execute(sql_weekly, [ts_code__])
                df_weekly = db_fetchdf(cursor)
            else:
            ...
            if DB_TYPE == 'doris':
                sql_monthly = translate_placeholder(translate_table('''
                ...
                '''))
                cursor = conn.cursor()
                cursor.execute(sql_monthly, [ts_code__])
                df_monthly = db_fetchdf(cursor)
            else:
...
def api_filter():
    ...
    try:
        # 执行筛选：表单参数翻译为 DSL，全部条件下推数据库执行
        List_filter_result__, Str_current_type__ = execute_form_filter_as_dsl(Parameter_dict__, metric_conditions, log_messages)
        log_messages.append(f"筛选共选出 {len(List_filter_result__)} 个证券")
...
def api_dsl_filter():
    ...
    # 方言必须与实际执行后端一致：DB_TYPE 决定连接，前端传入的 dialect 不可信
    dialect = DB_TYPE
