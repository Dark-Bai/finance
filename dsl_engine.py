...
class DorisDialect(Dialect):
    ...
    主要差异为标识符引号。任务6 迁移时在此继续补充表名映射等差异。"""
    name = 'doris'
    quote_char = '`'
...
class _Generator:
    ...
    def _gen_str(self, node):
        ...
        return f"'{escaped}'", 'string', node[2]
    ...
    def _gen_field(self, node):
        ...
        if name in BOOL_FIELDS:
            return self._field_sql(BOOL_FIELDS[name], None), 'bool', pos
        # 用户可能把函数名当字段用了
    ...
    def _gen_contains(self, node):
        ...
        return f"({lsql} LIKE '%' || {rsql} || '%')", 'bool', node[3]
    ...
    def build_select(self, where_sql):
        q = self.d.quote
        latest_cte = f'latest AS (\n    SELECT MAX({q("trade_date")}) AS d FROM {q("当日信息表")}\n)'
        hist_cte = ''
        hist_join = ''
        if self.hist_cols:
            cols = ',\n           '.join(
        ...
        return (
            ...
            f'FROM {q("基本信息表")} b\n'
            f'JOIN {q("当日信息表")} s ON s.{q("ts_code")} = b.{q("ts_code")}\n'
...
def execute_dsl(text, db_path, dialect='duckdb', limit=10000):
    """翻译并在 DuckDB 上执行，返回与现有筛选结果同构的行数据。
...
def get_dsl_help():
    ...
    return {
        ...
        'bool_fields': [
            {'name': 'ST', 'note': '风险警示股。排除写法：NOT ST（或 非 ST）'},
        ],
        'functions': [
